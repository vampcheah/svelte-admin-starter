...
```svelte
import {
  PageContainer, PageHeader, StatCard, DataTable, EmptyState,
  ConfirmDialog, ThemeToggle, LanguageToggle, Spinner, SearchInput, CommandMenu,
  type Column
} from '$lib/components/shared';
```

> 类型 `Column` 由 `DataTable.svelte` 导出并经 barrel 再导出。
...
| `value` | `string \| number` | （必填） | 否 | 主数值 |
| `icon` | `Component` | `undefined` | 否 | 右上角图标组件（如 Lucide 图标） |
| `change` | `number` | `undefined` | 否 | 带符号变化值，渲染为 `+12.5%` |
| `trend` | `'up' \| 'down' \| 'neutral'` | `undefined` | 否 | 显式趋势，优先于 `change` 推断 |
| `hint` | `string` | `undefined` | 否 | 趋势后的辅助文案（如 "vs last week"） |
| `class` | `string` | `undefined` | 否 | 追加类名 |

颜色规则：`up` → `text-emerald-500`，`down` → `text-red-500`，`neutral` → `text-muted-foreground`。
...
完整表格（搜索 + 选择 + toolbar + cell + actions）：
  ...
  {#snippet actions(row)}
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content align="end" class="w-36">
        ...
        <DropdownMenu.Item variant="destructive" onSelect={() => requestDelete(row)}>
