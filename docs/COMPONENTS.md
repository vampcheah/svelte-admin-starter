...
- [3. Shared 复用业务组件](#3-shared-复用业务组件)
  ...
  - [DatePicker / TimePicker / DateTimePicker](#datepicker--timepicker--datetimepicker)
...
<StatCard title="Revenue" value="$48,200" icon={DollarSign} change={12.5} hint="vs last week" />
```
...
```svelte
<script lang="ts">
	import { DatePicker, TimePicker, DateTimePicker } from '$lib/components/shared';
...
<TimePicker bind:value={time} name="startTime" hour12={false} />
<DateTimePicker bind:value={scheduledAt} name="scheduledAt" />
```
...
```svelte
import {(AppShell, AppSidebar, AppHeader, Breadcrumbs, NotificationsMenu, navGroups, findNavItem)} from
'$lib/shell'; import type {(NavItem, NavGroup)} from '$lib/shell';
```
